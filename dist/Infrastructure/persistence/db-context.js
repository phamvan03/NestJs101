...
let DbContext = class DbContext {
    constructor(config) {
        this.config = config;
        const tablePrefix = this.config.get('tablePrefix');
        this.user = dynamoose.model(`${tablePrefix}-users`, user_schema_1.UserSchema);
        this.task = dynamoose.model(`${tablePrefix}-tasks`, task_schema_1.TaskSchema);
    }
};
